  ...
  return <div className="h-dvh overflow-hidden bg-background text-foreground">
    ...
    <div className="grid h-[calc(100dvh-3.5rem)] min-w-0 lg:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 overflow-y-auto overflow-x-hidden pb-24">
        ...
        {screen === "new" && <NewMaterial courses={courses} chapters={chapters} topics={topics} online={online} generate={generate} onSaved={async (topic: StudyTopic) => { await cacheTopic(userId, topic); await load(); setActive(topic); setScreen("topic"); }} onError={setError} />}
        {screen === "topic" && active && <TopicView topic={active} chapters={chapters} online={online} regenerate={regenerate} onUpdate={async (payload: StudyPayload) => { const next = { ...active, generated_payload: payload }; setActive(next); setTopics((all) => all.map((t) => t.id === next.id ? next : t)); await cacheTopic(userId, next); }} onChanged={load} onReview={() => setScreen("review")} />}
...
function Tree({ courses, chapters, topics, expandedCourses, expandedChapters, setExpandedCourses, setExpandedChapters, onOpen, onChanged, online }: any) {
  ...
  async function rename(table: "courses" | "chapters" | "topics", id: string, current: string) { const name = window.prompt("New name", current); if (!name) return; if (table === "topics") await supabase.from("topics").update({ title: name }).eq("id", id); else if (table === "chapters") await supabase.from("chapters").update({ name }).eq("id", id); else await supabase.from("courses").update({ name }).eq("id", id); await onChanged(); }
...
function NewMaterial({ courses, chapters, topics, online, generate, onSaved, onError }: any) {
  const readPhotos = useServerFn(extractPhotoText);
  const suggestTitle = useServerFn(suggestTopicTitle);
  const [title, setTitle] = useState(""); const [text, setText] = useState(""); const [sourceType, setSourceType] = useState("notes"); const [sourceName, setSourceName] = useState<string | null>(null); const [chapterId, setChapterId] = useState(""); const [busy, setBusy] = useState(false); const [attaching, setAttaching] = useState(""); const [menuOpen, setMenuOpen] = useState(false);
  async function nameIfEmpty(value: string) { if (title.trim() || !value.trim()) return; try { const { title: suggested } = await suggestTitle({ data: { sourceText: value.slice(0, 20000) } }); if (suggested) setTitle(suggested); } catch { /* keep manual naming */ } }
  async function fileChanged(file?: File) { if (!file) return; setMenuOpen(false); setSourceName(file.name); const ext = file.name.split(".").pop()?.toLowerCase(); setSourceType(ext === "pdf" ? "pdf" : ext === "md" ? "md" : "txt"); if (!title) setTitle(file.name.replace(/\.[^.]+$/, "")); if (ext === "pdf") { setAttaching("Reading the PDF…"); try { const pdfjs = await import("pdfjs-dist"); pdfjs.GlobalWorkerOptions.workerSrc = new URL("pdfjs-dist/build/pdf.worker.min.mjs", import.meta.url).toString(); const doc = await pdfjs.getDocument({ data: await file.arrayBuffer() }).promise; let value = ""; for (let i = 1; i <= doc.numPages; i++) { const page = await doc.getPage(i); const content = await page.getTextContent(); value += content.items.map((item) => "str" in item ? item.str : "").join(" ") + "\n"; } if (!value.trim()) throw new Error(); setText(value); } catch { onError("This PDF is encrypted, scanned, or has no selectable text."); } finally { setAttaching(""); } } else { const value = await file.text(); setText(value); } }
  async function photosChosen(files: FileList | null) {
    if (!files?.length) return; setMenuOpen(false);
    if (!online) return onError("Connect to the internet to read text from photos.");
    if (files.length > 20) return onError("Choose up to 20 photos at a time.");
    const chosen = Array.from(files);
    setAttaching(`Reading text from ${chosen.length} photo${chosen.length > 1 ? "s" : ""}…`); onError("");
    try {
      const images = await Promise.all(chosen.map(async (file) => ({
        mimeType: file.type || "image/jpeg",
        data: await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result).split(",")[1] ?? ""); reader.onerror = () => reject(new Error("Could not read that photo.")); reader.readAsDataURL(file); }),
      })));
      const { text: extracted } = await readPhotos({ data: { images } });
      const merged = text.trim() ? `${text.trim()}\n\n${extracted}` : extracted;
      setText(merged); setSourceType("notes"); setSourceName(`${chosen.length} photo${chosen.length > 1 ? "s" : ""}`);
      await nameIfEmpty(merged);
    } catch (e) { onError(e instanceof Error ? e.message : "Could not read text from those photos."); }
  ...
  return <div className="mx-auto max-w-3xl p-4 sm:p-7"><p className="text-sm font-semibold text-primary">NEW MATERIAL</p><h1 className="mt-1 text-2xl font-bold">Turn material into a study guide</h1><div className="mt-6 space-y-5"><label className="block"><span className="mb-2 block text-sm font-medium">Topic title</span><Input className="h-11" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Photosynthesis" /></label>
    <div><span className="mb-2 block text-sm font-medium">Material</span><div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-2"><Textarea id="material-text" className="min-h-64 resize-y" value={text} onChange={(e) => { setText(e.target.value); setSourceType("notes"); }} placeholder="Paste the material you want to study…" />
      <Sheet open={menuOpen} onOpenChange={setMenuOpen}><SheetTrigger asChild><Button variant="outline" size="icon" className="h-11 w-11 shrink-0" aria-label="Attach material"><Paperclip /></Button></SheetTrigger><SheetContent side="bottom" className="p-4"><SheetHeader><SheetTitle>Add material</SheetTitle></SheetHeader><div className="mt-4 space-y-2 pb-4">
        <label className="flex min-h-14 cursor-pointer items-center gap-3 rounded-lg border px-4 text-sm font-medium"><Image className="shrink-0" />Photos (up to 20)<input className="sr-only" type="file" accept="image/*" multiple onChange={(e) => photosChosen(e.target.files)} /></label>
        <label className="flex min-h-14 cursor-pointer items-center gap-3 rounded-lg border px-4 text-sm font-medium"><FileText className="shrink-0" />PDF or text file<input className="sr-only" type="file" accept=".txt,.md,.pdf,text/plain,application/pdf" onChange={(e) => fileChanged(e.target.files?.[0])} /></label>
        <button type="button" className="flex min-h-14 w-full items-center gap-3 rounded-lg border px-4 text-left text-sm font-medium" onClick={() => { setMenuOpen(false); document.getElementById("material-text")?.focus(); }}><Type className="shrink-0" />Paste text</button>
      </div></SheetContent></Sheet></div>
      <p className="mt-2 min-h-5 text-xs text-muted-foreground">{attaching || (sourceName ? `Attached: ${sourceName}` : "Paste text, or use the clip to add photos or a file.")}</p></div>
    <div><span className="mb-2 block text-sm font-medium">Save in chapter</span>{chapters.length ? <Select value={chapterId} onValueChange={setChapterId}><SelectTrigger className="h-11"><SelectValue placeholder="Choose a chapter" /></SelectTrigger><SelectContent>{courses.flatMap((course: Course) => chapters.filter((ch: Chapter) => ch.course_id === course.id).map((chapter: Chapter) => <SelectItem key={chapter.id} value={chapter.id}>{course.name} / {chapter.name}</SelectItem>))}</SelectContent></Select> : <Button variant="outline" className="h-11 w-full" onClick={createFolders}><FolderPlus />Create course and chapter</Button>}</div><Button className="h-12 w-full" disabled={busy || !!attaching || !title.trim() || text.trim().length < 20 || !online} onClick={submit}>{busy ? <><RefreshCw className="animate-spin" />Building your lesson…</> : <><Sparkles />Generate study guide</>}</Button></div></div>;
}
